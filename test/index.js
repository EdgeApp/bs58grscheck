...
fixtures.valid.forEach(function (f) {
  tape('decodes ' + f.string, function (t) {
    t.plan(2)
    var actual = bytesToHex(bs58grscheck.decode(f.string))
    t.equal(actual, f.payload)

    actual = bytesToHex(bs58grscheck.decodeUnsafe(f.string))
    t.equal(actual, f.payload)
  })
})
...
fixtures.valid.forEach(function (f) {
  tape('encodes ' + f.string, function (t) {
    ...
    var actual1 = bs58grscheck.encode(u8)
    var actual2 = bs58grscheck.encode(array)
    var actual3 = bs58grscheck.encode(buffer)
